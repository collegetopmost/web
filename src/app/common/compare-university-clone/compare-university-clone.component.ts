...
export class CompareUniversityCloneComponent {
 // Sample data for demonstration
  ...
  universityList:any=[]
    getUniversityListName() {
    this.universityList = [];

    this.api.getapi('getUniversityListName').subscribe(
      (res: any) => {
        console.log('getUniversityListName', res?.data);
...
searchInstitute(uniNumber:any){
       const dialogRef =  this.dialog.open(SearchUniversityComponent, {
    width: '460px',
    maxWidth: '95vw',
    maxHeight: '95vh',
    height: 'auto',
    ...
    data: {list:this.universityList}
      ...
      if(result?.university){
        let findAleradyIn=this.selectedUniversity.find((f:any)=>f?.university?.id==result?.university?.id)
        if(!findAleradyIn){
this.selectedUniversity[uniNumber].university=result?.university
        }else{
     this.openDialog()
        }
...
getLengthOfSelected(){
  return this.selectedUniversity.filter((f:any)=>f.university!=null)?.length
}
  clearAll() {
    this.selectedUniversity?.forEach((f:any)=>{
      f.university=null
    })
